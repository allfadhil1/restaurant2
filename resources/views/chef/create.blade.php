...
<div class="max-w-2xl mx-auto mt-10 p-6 bg-white rounded-xl shadow-md">
    ...
    <form 
        action="{{ isset($chef) ? route('chef.update', $chef) : route('chef.store') }}"
        method="POST"
    ...
    >
        ...
        <div>
            ...
            <input 
                type="text" 
                name="specialty" 
                value="{{ old('specialty', $chef->specialty ?? '') }}"
                class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#fb5849]" 
                required
            >
        </div>
        ...
        <div>
            ...
            @if(isset($chef) && $chef->gambar)
                <div class="mt-3">
                    ...
                    <img src="{{ asset('storage/' . $chef->gambar) }}" alt="Foto Chef" class="h-32 rounded-lg shadow-md border">
                </div>
            @endif
        </div>
